...
function fillModal(id) {
  switch(id) {
    ...
    case "popupDNACollection":
      ...
      $("#modalBody").html(`<p>For the purposes of family verification at the border, immigration officials conduct Rapid DNA analysis. Authorized testers collect DNA samples from each family member via cheek swab. Each swab contains a RFID tracking chip that ensures “that the DNA sample is attributed to the correct person.” Samples are later destroyed during the Rapid DNA analysis. </p>
      `);
      break;
      ...
      case "popupSharepoint":
        ...
        $("#modalBody").html(`<p>SharePoint log entries for migrants contain personally identifiable information, including names and dates of birth for both parent and child, alien registration numbers, DNA test results and the barcode number that corresponds to the DNA sample.</p><br><p>SharePoint files are accessible by ICE officials on a “need to know” basis. DNA test results are retained for 75 years. </p>
        `);
      break;

      case "popupInvestigation":
        $("#modalTitle").text("Investigative Case Management System ");
        $("#modalBody").html(`<p>In the event that ICE “chooses to investigate or prosecute an individual, that information will be stored in” the Investigative Case Management system. </p><br><p>Information stored in ICM is retained indefinitely (for at least 20 years following the closure of the case) and can be shared with outside agencies. </p>
        `);
      break;

      case "popupProfile":
        $("#modalTitle").text("DNA Profile");
        $("#modalBody").html(`<p>A DNA profile identifies a handful of specific genetic markers from your DNA. These markers can be compared to markers in the same location in other DNA profiles to determine if a genetic match exists. </p> <br> <p> [Disclaimer about what these markers can be used for?]</p>
        `);
      break; 
